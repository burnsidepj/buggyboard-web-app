...
3. The same delivery **must** include Playwright tests that cover the feature’s new behavior. Follow `specs/engineering/test-automation-patterns.md`. Do not pause for review until those tests are written and passing (unless the human explicitly defers them).
...
- Progress is tracked in **`specs/PROGRESS.md`** as checklists (e.g. spec written, backend done, frontend done, Playwright tests, linter/errors resolved, review done) per feature.
